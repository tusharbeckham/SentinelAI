...
export function NetworkHero({ report, live }: { report: Bundle["report"]; live: LiveStatus }) {
	...
	useEffect(() => {
		...
		/*
		 * A SOLID, not a glow. The previous material was additive with no depth
		 * write and fresnel-only shading, which can only ever produce a bright
		 * outline -- that is why the object read as a ring. This one writes
		 * depth, blends normally, renders front faces only, and takes its
		...
		const hullMat = new THREE.ShaderMaterial({
			...
			vertexShader: [
				...
				"uniform float uRespond; uniform float uCutY;",
				"varying vec3 vPos; varying vec3 vView; varying float vY;",
				"void main() {",
				"  vec3 n = normalize(normal);",
				"  float b = sin(uTime * 0.45 + position.x * 0.62 + position.y * 0.83);",
				"  vec3 p = position + n * uOpen * (0.55 + 0.30 * b);",
				"  float fold = uFold * (1.0 - uAxis);",
				"  p.y *= mix(1.0, 0.13, fold);",
				"  p.xz *= mix(1.0, 1.34, fold);",
				"  float bands = 7.0;",
				"  float q = floor(p.y / 7.4 * bands + 0.5) / bands * 7.4;",
				"  p.y = mix(p.y, q, uAxis * 0.85);",
				"  p.xz *= mix(1.0, 0.94, uAxis);",
				"  float ang = uTwist * 2.30 * (p.y / 7.4);",
				"  float ca = cos(ang); float sa = sin(ang);",
				"  p.xz = mat2(ca, sa, -sa, ca) * p.xz;",
				"  float sideSign = step(uCutY, p.y) * 2.0 - 1.0;",
				"  p.y += sideSign * uSplit * 1.90;",
				"  p *= 1.0 - uRespond * 0.34;",
				"  vY = p.y;",
				"  vec4 mv = modelViewMatrix * vec4(p, 1.0);",
				"  vPos = mv.xyz;",
			...
			fragmentShader: [
				"uniform vec3 uRim; uniform vec3 uWarn; uniform vec3 uAlarm;",
				"uniform float uOpacity; uniform float uCut; uniform float uCutY;",
				"uniform float uRespond; uniform float uOpen;",
				"varying vec3 vPos; varying vec3 vView; varying float vY;",
				"void main() {",
				"  vec3 nrm = normalize(cross(dFdx(vPos), dFdy(vPos)));",
				"  vec3 key = normalize(vec3(0.45, 0.80, 0.55));",
				"  vec3 fill = normalize(vec3(-0.60, -0.25, 0.35));",
				"  float kd = max(dot(nrm, key), 0.0);",
				"  float fd = max(dot(nrm, fill), 0.0);",
				"  float f = 1.0 - abs(dot(nrm, normalize(vView)));",
				"  float rim = pow(f, 3.0);",
				"  vec3 base = vec3(0.055, 0.072, 0.100);",
				"  vec3 c = base * (0.30 + 1.55 * kd) + uRim * 0.13 * fd;",
				"  c += uRim * rim * 1.15;",
				"  float d = vY - uCutY;",
				"  float band = exp(-d * d * 3.2) * uCut;",
				"  float above = smoothstep(-0.35, 0.35, d) * uCut;",
				"  c = mix(c, c + uWarn * 0.55, above);",
				"  c += uWarn * band * 1.35;",
				"  c = mix(c, uAlarm * (0.16 + 0.55 * kd), uRespond * 0.65);",
				"  float a = uOpacity * (0.90 + 0.10 * rim);",
				"  a *= 1.0 - uOpen * 0.15;",
				"  gl_FragColor = vec4(c, a);",
				"}",
			].join("\n"),
